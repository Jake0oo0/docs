...
Instead of overriding the existing attributes, the `merge` method may be used to only change specific values.

[source, javascript]
```
const post = yield Post.findBy('id', 1)
post.fill({title: 'A Blog Post', body: 'Adding some new content'})

post.merge({title: 'Cool Blog Post'}) // Only changes 'title' attribute
